...
! function(a) {
    ...
    document.head.appendChild(function() {
        ...
        return a.href = b + "../layer/layer.css", a.type = "text/css", a.rel = "styleSheet", a.id = "layermcss", a
    }());
    var g = {
        type: 0,
        shade: !0,
    ...
    var k = {
        ...
        alert:function(a,t,f){
            k.open({
                content: a,
                style: 'background-color: rgba(0,0,0,.6); text-align:center; color:#fff; border:none; font-size:20px; letter-spacing: 4px; line-height: 200%;max-width: 300px',
                time: t?t:2
            });
            if (f && f!=undefined) setTimeout(f,t*1000);
        },
        confirm:function(msg,func,okValue,cancelVal){
            layer.open({
                content: '<div style="font-size: 18px; font-weight: 300; color: #4A4747; margin-top: 18px; margin-bottom: 10px;">&nbsp;'+msg+'</div>',
                 btn: [okValue?okValue:'确定', cancelVal?cancelVal:'取消'],
                style:'min-width: 220px;min-height:125px;text-align: center;border-radius: 6px;',
                shadeClose: true,
                yes: function(index) {
                    layer.close(index);
                    func();
